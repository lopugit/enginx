...
let scripts = {
	...
	certbotcerts: function(args){
		...
		return `
			...
			--entrypoint "certbot" \
			certbot \
			certonly --webroot -w /things/perm/certbotwww ${args.staging || ''} ${args.dryrun ? '--dry-run' : ''} --email ${args.email} ${args.domains} --rsa-key-size ${args.rsa_key_size || '4096'} --agree-tos --force-renewal  ;
		`
	},
	certbotautorenew: `
		docker rm --force certbotautorenew ;
		docker run ${args.i ? '-i' : '-d'} --name \
		certbotautorenew \
		-v certbotwww:/things/perm/certbotwww \
		-v certbotconf:/etc/letsencrypt \
		-v certbotCerts:/etc/letsencrypt/live \
