...
function generateConfigs(args){
	let out = args.things ? '/things' : './output'
	let tmp = out+'/tmp'
	let perm = out+'/perm'
	let dirs = {
		sites: tmp+'/sites/',
	...
	for(var site of sites){
		...
		if(site.self_signed){
			...
			try {
				...
				try {
					...
					console.log('generating certs failed')
				}
				let certpath = `${dirs.certs}${site.server_name}/crt.priv.pem`
		...
		if(cert){
			...
			if(site.ip || site.ips){
				var upstream = Math.round(Math.random()*10000000)
				// ip_hash;
				file += `
					upstream ${upstream} {
				`
			...
			server {
				...
				ssl_certificate         ${args.dev ? dirs.localcertsosx : (site.certbot ? dirs.certbotcerts : dirs.certs)}${site.server_name}/${site.certbot ? 'fullchain.pem' : 'crt.pub.pem'} ;
				ssl_certificate_key     ${args.dev ? dirs.localcertsosx : (site.certbot ? dirs.certbotcerts : dirs.certs)}${site.server_name}/${site.certbot ? 'privkey.pem' : 'crt.priv.pem'} ;
			`
			if(site.certbot){
				file += `
			...
			if(site.ip || site.ips){
				file += `
					location / {
						...
						proxy_pass ${site.protocol || 'http://'}${upstream} ;
				`
				if(site.changeHost){
