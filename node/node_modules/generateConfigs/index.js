...
function generateConfigs(args){
	...
	let dirs = {
		...
		localcertsosx: '/usr/local/etc/openresty/',
	}
	let paths = {
		...
		if(site.self_signed){
			...
			if(cert){
				...
				server {
					...
					ssl_certificate         ${args.dev ? dirs.localcertsosx : dirs.certs}certs/${site.server_name}/crt.pub.pem ;
					ssl_certificate_key     ${args.dev ? dirs.localcertsosx : dirs.certs}certs/${site.server_name}/crt.priv.pem ;
				`
				if(site.ip){
					file += `
						location / {
